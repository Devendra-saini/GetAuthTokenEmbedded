...
async function getAllReportsInfo() {

    // Get the Report Embed details
    try {

        // Get report details and embed token
        const embedParams = await getEmbedParamsForAllReports(config.workspaceId);
...
async function getEmbedParamsForAllReports(workspaceId){
 ...
 */
async function getEmbedInfoForGivenId(reportId)
{
    try {

        // Get report details and embed token
        const embedParams =  await getEmbedParamsForSingleReport(reportId);
...
async function getEmbedParamsForSingleReport(reportId, additionalDatasetId) {
    const reportInGroupApi = `https://api.powerbi.com/v1.0/myorg/groups/${config.workspaceId}/reports/${reportId}`;
    const headers = await getRequestHeader();
    ...
    reportEmbedConfig.embedToken = await getEmbedTokenForSingleReportSingleWorkspace(reportId, datasetIds, config.workspaceId);
...
module.exports = {
    ...
    getAllReportsInfo:getAllReportsInfo,
    getEmbedInfoForGivenId:getEmbedInfoForGivenId
}